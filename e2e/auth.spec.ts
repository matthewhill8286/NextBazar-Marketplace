...
test.describe("Login page", () => {
  ...
  test("renders the login form", async ({ page }) => {
    await expect(
      page
        .getByRole("heading", { name: /sign in|log in|welcome back/i })
        .first(),
    ).toBeVisible();
  });
  ...
  test("shows a validation error when submitting empty form", async ({
    page,
  }) => {
    await page
    ...
    const errorVisible = await page
      .getByText(/required|invalid|enter your email/i)
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe("Register page", () => {
  ...
  test("renders the register form", async ({ page }) => {
    await expect(
      page
        .getByRole("heading", { name: /register|create account|sign up/i })
        .first(),
    ).toBeVisible();
  });
  ...
  test("has a link back to the login page", async ({ page }) => {
    await expect(
      page
        .getByRole("link", { name: /sign in|log in|already have an account/i })
        .first(),
    ).toBeVisible();
  });
});
...
test.describe("Auth redirect behaviour", () => {
  test("visiting /saved when not signed in redirects to login", async ({
    page,
  }) => {
    await mockSupabase(page);
  ...
  test("visiting /notifications when not signed in redirects to login", async ({
    page,
  }) => {
    await mockSupabase(page);
  ...
  test("visiting /post when not signed in redirects to login", async ({
    page,
  }) => {
    await mockSupabase(page);
  ...
  test("visiting /messages when not signed in redirects to login", async ({
    page,
  }) => {
    await mockSupabase(page);
