...
import { mockSupabase, mockListings } from "./helpers/mocks";
...
test.describe("Listing detail page", () => {
  ...
  test("shows the listing title", async ({ page }) => {
    await expect(page.getByText(listing.title)).toBeVisible({ timeout: 10_000 });
  });

  test("shows the formatted price", async ({ page }) => {
    await expect(page.getByText(/€800/).first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("shows the location", async ({ page }) => {
    await expect(page.getByText("Nicosia").first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("shows the condition", async ({ page }) => {
    await expect(
      page.getByText(/used|good|like new/i).first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows the description", async ({ page }) => {
    await expect(
      page.getByText(/Excellent condition/).first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows the 'Send Message' contact button", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: /message|send message/i })
        .or(page.getByText(/send message/i))
        .first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows WhatsApp contact button when seller has a number", async ({ page }) => {
    await expect(
  ...
  test("shows the verified badge for a verified seller", async ({ page }) => {
    await expect(
      page.locator('[data-testid="verified-badge"]')
  ...
  test("shows 404 state for an unknown slug", async ({ page }) => {
    ...
    await expect(
      page.getByText(/not found|removed/i).first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows safety tips section", async ({ page }) => {
    await expect(
      page.getByText(/safety tips/i).or(page.getByText(/meet in a public/i)).first()
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows view count", async ({ page }) => {
    await expect(page.getByText(/42|views/i).first()).toBeVisible({ timeout: 10_000 });
  });
});
...
test.describe("Listing detail — featured listing", () => {
  ...
  test("shows Negotiable badge for a negotiable listing", async ({ page }) => {
    await mockSupabase(page);
    await page.goto(`/listing/${mockListings[1].slug}`);
    await expect(page.getByText("Negotiable").first()).toBeVisible({ timeout: 10_000 });
  });
});
